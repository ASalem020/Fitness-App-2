...
export function RangeSlider({
  ...
  return (
    <div className="flex w-full flex-col items-center justify-center space-y-4 py-8 select-none">
      <div className="text-sm font-semibold text-[#FF4A11] uppercase tracking-wider">{label}</div>
      <div className="relative w-full max-w-sm overflow-hidden [mask-image:linear-gradient(to_right,transparent,black_20%,black_80%,transparent)]" ref={emblaRef}>
        <div className="flex touch-pan-x items-center py-4">
          {values.map((val, index) => {
            ...
            return (
              ...
              >
                <span className={cn("font-bold transition-all duration-200 transform", scaleClass)}>
                  {val}
                </span>
              </div>
            )
